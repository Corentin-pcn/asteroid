...
def pos_Ast(t):
    ...
    e = 0.3896
    n = 0.01720209895/np.sqrt(a**3)
    i = np.deg2rad(-108.358)
    Om = np.deg2rad(-276.509)
    om = np.deg2rad(-226.107)
    t0 = 5055.5
    
    M0 = np.deg2rad(146.88)
    ...
    B = np.array([Xd, Yd, 0])
    x = np.matmul(R3(Om),np.matmul(R1(i),np.matmul(R3(om),A)))
    y = np.matmul(R3(Om),np.matmul(R1(i),np.matmul(R3(om),B)))
...
ax.plot(Xj,Yj,Zj)
ax.plot([i[0] for i in R],[i[1] for i in R],[i[2] for i in R])
